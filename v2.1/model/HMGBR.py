...
class HMGBR(nn.Module):
    ...
    def __init__(self, in_dimension, hidden_dimension, out_dimension, device, args):
        ...
        if args.model == 'HMGBR':
            ...
            if '-G' in args.model:
                # func error off
                self.mtl = AdjMTLModel(num_feature=6*hidden_dimension, h_units=256, num_experts=6, selectors=2, tower_h=64)
            else:
        ...
        if '-A' in args.model:
            ...
            if '-G' in args:
                # func error off
                self.mtl = MTLModel(num_feature=6*hidden_dimension, h_units=256, num_experts=6, selectors=2, tower_h=64)
            else:
    ...
    def forward(self, target_user, item_sample, user_sample):
        ...
        if '-G' in self.option:
            init_item_embed = init_item_embed_hgcn
            part_item_embed = part_item_embed_hgcn
            init_part_embed = init_part_embed_hgcn
        ...
        loss = bprloss + alpha1 * task1_listloss + alpha2 * task2_bpr2
